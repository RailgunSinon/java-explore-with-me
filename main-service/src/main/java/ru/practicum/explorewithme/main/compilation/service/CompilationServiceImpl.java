...
import ru.practicum.explorewithme.main.compilation.controller.mapper.CompilationMapper;
...
import ru.practicum.explorewithme.main.compilation.repository.CompilationRepository;
...
public class CompilationServiceImpl implements CompilationService {
    ...
    public CompilationDto addNewCompilation(NewCompilationDto newCompilationDto) {
        ...
        Compilation compilation = CompilationMapper
            .toCompilation(newCompilationDto, compilationEvents);
    ...
    @Override
    @Transactional
    public CompilationDto updateCompilationInfo(Long compilationId,
    ...
    @Override
    public Collection<CompilationDto> getAllCompilations(Boolean pinned, Integer from,
        Integer size) {
        Pageable pageable = PageRequest.of(from / size, size, Sort.by("id").ascending());

        log.info("Получен список подборок с параметрами pinned {}, from {}, size {}", pinned, from,
            size);
        return pinned != null ? CompilationMapper
            .toCompilationDtoList(compilationRepository.findAllByPinnedEquals(pinned, pageable))
